...
//In this first import statement, you'll see that we imported the following two functions:
...
//As of right now, our StoreContext is nothing but an empty container waiting for data to be provided to it as state
...
//The last thing we need to do is export both our StoreProvider and useStoreContext() functionality. At the bottom of the GlobalState.js file, add the following code:

export { StoreProvider, useStoreContext }; 

//Great, everything for our global state object is now in place! Before we get to refactoring our components, let's update App.js to make our global state available to all of our components

//As we can see, we have successfully created our global state object, but none of it affects or is affected by our components just yet. But we now know that we can use our useStoreContext() function to grab the state from the <StoreProvider> component and use the returning dispatch method to update it—great job!